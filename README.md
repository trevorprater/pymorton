...
In the context of linearizing K-dimensional integer coordinates, *Morton numbers* are very useful: **`{x, y, ..., K}`** are combined into a single ordinal value that is easily compared, searched, and indexed against other *Morton numbers*, where the inputs, **`{x, y, ..., K}`**, exist in the domain of natural numbers, **`{x, y, ..., K ∈ N}`**.
